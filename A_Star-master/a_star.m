...
environment(xval,yval)=0;
%Initialize environment with location of the target
...
% make it invalid for further iterations
...
while((cell_x ~= x_target || cell_y ~= y_target) && path_not_found == 1)
    ...
    %由当前点目标点障碍物和地图边界 对该点的八邻域进行搜索获取到八个点对应的相应的信息
    successors_size=size(successors,1);
    
    for i=1:successors_size
        flag=0;
        for j=1:valid_size
            if(successors(i,1) == valid(j,2) && successors(i,2) == valid(j,3) )
                % if successor same as already existing cell inpath
                %             disp('valid')
                %             valid
                %             disp(' ');
                valid(j,8)=min(valid(j,8),successors(i,5)); 
                % check for minimum f and then pick it
    ...
    % 获取f最小的编号 得到下一个要去的点
    if (index_min_cell ~= -1) 
        % if index with minimum fn is obstacle no path exists
        % 更新新的起始点
...
end_size=size(in_valid,1);
path=[];
xval=in_valid(end_size,1); 
% pick last in in_valid_list that must be target
yval=in_valid(end_size,2);
i=1;
path(i,1)=xval;
path(i,2)=yval;
i=i+1;
